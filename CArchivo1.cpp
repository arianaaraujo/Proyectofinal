...
void CArchivo::guardar_ext(string cache) {
	...
	if (ext == ".rtf" || ext == "rtf" || ext == ".RTF" || ext == "RTF") {
		archivo.open("texto.rtf");
		if (archivo.is_open()) {
			for (int i = 0; i < cache.size(); i++) {
				archivo << cache[i];
			}
...
void CArchivo::opciones(string nombre, string cache, string cache2, vector<vector<string>> cache_vector, vector<vector<string>> cache_vector2, int anterior, int limite) {
	...
	case 10:
		if (cache2 != cache)cache2 = cache;
		guardar_ext(cache2); cout << "Se guardó correctamente, revise sus archivos." << endl; opciones(nombre, cache, cache2, cache_vector, cache_vector2, 10, limite); break;
